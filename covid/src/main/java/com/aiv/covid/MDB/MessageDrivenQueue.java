...
@Slf4j
public class MessageDrivenQueue implements MessageListener {

    @EJB
    RegionDao regionBean;

    @Override
    public void onMessage(Message message) throws PersistenceException {
        try{
            if(message instanceof MapMessage) {
                MapMessage mapMessage = (MapMessage) message;
                log.info("[AIVMessageDrivenQueue] MESSAGE HAS BEEN RECEIVED");
                ...
                if(mapMessage.getString("action").equals("addData")){
                    ...
                    log.info("[AIVMessageDrivenQueue] Data: " + data.toString());
                    log.info("[AIVMessageDrivenQueue] Added for region: " + region.getUuid());
                }else{
                    log.info("[AIVMessageDrivenQueue] ACTION IS NOT VERIFIED");
                }
            }else{
                log.info("[AIVMessageDrivenQueue] UNKNOWN MESSAGE HAS BEEN RECEIVED");
            }
